...
function _comisionLineChart(puntos, meta, diasEnMes, diaCorte, esMesActual){
  ...
  var metaHoyMarker = '', metaHoyLabel = '', metaHoyY = null;
  if (esMesActual && diaCorte >= 1 && diaCorte <= diasEnMes){
    var metaHoyValor = meta * diaCorte / diasEnMes;
    metaHoyY = yFor(metaHoyValor);
    metaHoyMarker = '<circle cx="' + dotX + '" cy="' + metaHoyY + '" r="3" fill="var(--verde)"/>';
  }

  var realY, metaHoyLabelY;
  if (metaHoyY !== null){
    var realEsSuperior = dotY <= metaHoyY;
    realY = realEsSuperior ? (dotY - 11) : (dotY + 16);
    metaHoyLabelY = realEsSuperior ? (metaHoyY + 14) : (metaHoyY - 9);
  } else {
  ...
  return '<svg viewBox="0 0 ' + w + ' ' + h + '" class="com-chart" preserveAspectRatio="none">' +
    ...
    metaLabel + realLabel + metaHoyLabel +
    '</svg>';
}
