...
function _comisionLineChart(puntos, meta, diasEnMes, diaCorte, esMesActual){
  var w = 400, h = 145, padL = 4, padR = 6, padT = 26, padB = 18;
  var plotW = w - padL - padR, plotH = h - padT - padB;
  var ultimoReal = puntos.length ? puntos[puntos.length - 1].real : 0;
  var maxVal = Math.max(meta, ultimoReal, 1) * 1.08;
  ...
  var metaX = xFor(diasEnMes), metaY = yFor(meta);
  ...
  if (esMesActual && diaCorte >= 1 && diaCorte <= diasEnMes){
    ...
    hoyMarker = '<line x1="' + hx + '" y1="' + padT + '" x2="' + hx + '" y2="' + (padT + plotH) + '" stroke="var(--texto-sub)" stroke-width="1" stroke-dasharray="2 2" opacity="0.6"/>';
  }
  ...
  // Siempre a un LADO del punto (nunca centrada encima) -- el punto de hoy
  // cae justo sobre la línea vertical punteada de "hoy", y una etiqueta
  // centrada ahí queda partida en dos por la línea, ilegible.
  var realArriba = dotY > padT + 20;
  var realY = realArriba ? (dotY - 11) : (dotY + 16);
  var haciaLaDerecha = dotX < plotW * 0.62;
  var realAnchor = haciaLaDerecha ? 'start' : 'end';
  var realX = haciaLaDerecha ? (dotX + 7) : (dotX - 7);
  var realLabel = '<text x="' + realX + '" y="' + realY + '" class="com-chart-label com-chart-label-real" text-anchor="' + realAnchor + '">Hoy ' + _copCorto(ultimoReal) + '</text>';
